...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  // cos α =  	скалярное произведение 	 /  (модуль вектора * модкль вектора)
}
...
function getFibonacciNumber(index) {
  if (index < 0) return undefined; // Обработка случая отрицательного индекса
  if (index === 0) return 0; // вариант для индекса 0
  if (index === 1) return 1; // вариант для индекса 1
  ...
  for (let i = 2; i <= index; i++) {
    ...
    a = b; //  следующему числу Фибоначчи
    b = fib; // Обновите b до последнего числа Фибоначчи.
  }

  return fib; // Вернуть число Фибоначчи по указанному индексу
}
...
//Используйте формулу (n×(n+1))/2
// который эффективно вычисляет сумму первых nn натуральных чисел.
...
function getSumOfDigits(num) {
  ...
  return String(Math.abs(num)) // Обрабатывайте отрицательные числа, принимая абсолютное
    .split('')
 ...
 *   15  => false
 */
//Побитовая операция : выражение (num & (num - 1)) === 0 эффективно проверяет, является ли число степенью двойки:
//Это работает, потому что в двоичном представлении степени двойки установлен ровно один бит. Например, 44 это 100100 в двоичном формате и 33 это 011011 . Когда вы вычитаете 11 из 44 , ты получишь 33, который переворачивает все биты после крайнего правого 11 , таким образом, 100 и 011 = 000 .
function isPowerOfTwo(num) {
  if (num <= 0) return false; // положительные числа.
  return (num & (num - 1)) === 0; // Проверка является ли число степенью двойки
}
...
function toFixed(number, fractionDigits) {
  return number.toFixed(fractionDigits); // Use the built-in toFixed method
}
